...
(function () {
  ...
  const state = {
    ...
    // Timer
    timerSeconds: 132 * 60, // 2 hours 12 mins (132 mins for 44 questions x 2 = 264 mins or 132 mins session)
    timerElapsed: 0,
    timerInterval: null,
    isTimerRunning: true,
    
    // Subset mode (e.g. retaking incorrect questions)
  ...
  function renderCurrentSection() {
    ...
    el.sessionTagBadge.textContent = sec.session;
    el.sectionProgressTag.textContent = `Case ${state.currentSectionIndex + 1} of ${state.examData.length}`;
    ...
    el.btnBottomPrevSec.disabled = state.currentSectionIndex === 0;
    el.btnNextSection.disabled = state.currentSectionIndex === state.examData.length - 1;
    el.btnBottomNextSec.disabled = state.currentSectionIndex === state.examData.length - 1;

    // Questions Range
    const firstQ = sec.questions[0];
    const lastQ = sec.questions[sec.questions.length - 1];
    el.vignetteQRange.textContent = `Questions #${firstQ.id} – #${lastQ.id}`;
  ...
  function renderVignetteContent(vignetteItems) {
    ...
    vignetteItems.forEach(item => {
      if (item.type === 'text') {
        ...
        caption.innerHTML = '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/><line x1="11" y1="8" x2="11" y2="14"/><line x1="8" y1="11" x2="14" y2="11"/></svg> Томруулж харах';
    ...
    questions.forEach((q, qIndex) => {
      ...
      if (state.mode === 'practice' && selectedChoice) {
        ...
        solHeader.innerHTML = isCorrect 
          ...
          : `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg> Буруу! (Таны сонголт: ${selectedChoice}, Зөв хариулт: ${q.correct_answer})`;
        solCard.appendChild(solHeader);

        const solDetails = document.createElement('div');
        solDetails.className = 'sol-details';
        
        // Show explanation for correct answer + chosen answer if different
  ...
  function renderDrawer() {
    ...
    state.examData.forEach((sec, sIdx) => {
      ...
      secHead.innerHTML = `
        ...
        <span style="font-size:0.75rem; color:var(--text-muted);">${sec.session === 'Morning Session' ? 'Morn' : 'Aft'}</span>
      `;
  ...
  function computeAndRenderResults() {
    ...
    if (pct >= 70) {
      el.scoreStatusBadge.textContent = "Estimated Passing Score (70%+)";
      ...
      el.scoreHeadline.textContent = "Баяр хүргэе! Та амжилттай давлаа";
      el.scoreSummaryDesc.textContent = `Та CFA Level II жишиг шалгалтад ${pct}% авч тэнцэх түвшинд хүрлээ. Доорх алдаа шинжилгээний хэсгээс алдсан ${incorrectCount} асуултын нарийн тайлбарыг нягтална уу.`;
    } else if (pct >= 60) {
      el.scoreStatusBadge.textContent = "Borderline Score (60–69%)";
      ...
      el.scoreHeadline.textContent = "Хангалттай ойрхон байна (Давтах шаардлагатай)";
      el.scoreSummaryDesc.textContent = `Та нийт ${pct}% авсан байна. Тэнцэх босго (70%+) руу хүрэхийн тулд алдсан сэдвүүдээ доорх тайлбаруудаас сайтар уншиж дахин ажиллана уу.`;
    } else {
      ...
      el.scoreHeadline.textContent = "Сэдвүүдийг дахин давтах шаардлагатай";
      el.scoreSummaryDesc.textContent = `Та нийт ${pct}% авсан байна. Доорх "Зөвхөн алдсан асуултууд" хэсгээс бодолтын арга, томьёо болон алдааны үндэслэлийг судалж дахин дадлага хийнэ үү.`;
    }
    ...
    // Default to 'incorrect' filter for high impact mistake review
  ...
  // --- MISTAKE REVIEW RENDERING ("Юу нь дээр алдсан") ---
  ...
  function renderReviewQuestionsList() {
    ...
    if (filtered.length === 0) {
      el.reviewQuestionsList.innerHTML = `
        <div style="text-align: center; padding: 3rem; background-color: var(--bg-subtle); border-radius: var(--radius-lg); border: 1px dashed var(--border-color);">
          <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="color: var(--success); margin-bottom: 0.5rem;"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
          <h4 style="font-weight: 700; margin-bottom: 0.25rem;">Энэ шүүлтүүрт тохирох асуулт олдсонгүй</h4>
          <p style="font-size: 0.88rem; color: var(--text-secondary);">Та бүх асуултаа зөв хариулсан байна эсвэл өөр шүүлтүүр сонгоно уу.</p>
        </div>
      `;
      return;
    }
    ...
    filtered.forEach(({ sec, q }) => {
      ...
      qNum.textContent = `Q#${q.id} (Item ${q.section_question_number}/4)`;
      leftTop.appendChild(qNum);

      const vTitle = document.createElement('span');
      vTitle.className = 'rc-vignette-title';
      vTitle.textContent = `${sec.title} &bull; ${q.topic || q.topic_area || 'Finance'}`;
      ...
      statusPill.innerHTML = isCorrect
        ? `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg> Зөв (+1)`
        : `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg> Алдсан`;
      ...
      q.choices.forEach(ch => {
        ...
        if (isChoiceCorrect) {
          badgeTag.innerHTML = `<span style="font-size:0.78rem; font-weight:700; color:var(--success); background:var(--success-bg); border:1px solid var(--success-border); padding:2px 8px; border-radius:12px;">Зөв сонголт</span>`;
        } else if (isUserChoice) {
          badgeTag.innerHTML = `<span style="font-size:0.78rem; font-weight:700; color:var(--danger); background:var(--danger-bg); border:1px solid var(--danger-border); padding:2px 8px; border-radius:12px;">Таны буруу сонголт</span>`;
        }
        ...
        // Explanation body for this choice
        ...
        if (expItems.length > 0) {
          ...
          expItems.forEach(item => {
            if (item.type === 'text') {
              ...
              img.src = item.src;
              img.loading = 'lazy';
              img.title = 'Томруулж харах бол дарна уу';
  ...
  function retakeIncorrectQuestions() {
    // Clear user answers for only the wrong questions
    state.examData.forEach(sec => {
      sec.questions.forEach(q => {
    ...
    saveState();
    state.mode = 'practice'; // Set to practice mode for instant learning
    el.btnModePractice.classList.add('active');
    el.btnModeExam.classList.remove('active');

    // Switch view back to questions
